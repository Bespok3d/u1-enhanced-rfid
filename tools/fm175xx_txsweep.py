...
MODEWIDTH = 0x24; RFCFG = 0x26; GSNON = 0x27; CWGSP = 0x28; GSNOFF = 0x23; MODGSP = 0x29
...
class FM:
    ...
    def init(s, gsnon, cwgsp, modgsp, gsnoff, gain=0x68, rxth=0x84):
        # vary TX driver conductance / modulation depth (all within the chip's rated range).
        s.rw(TXMODE, 0x00); s.rw(RXMODE, 0x08); s.rm(TXAUTO, 0x40, SET); s.rw(MODEWIDTH, 0x26)
        s.rw(CONTROL, 0x10); s.rw(GSNON, gsnon); s.rw(CWGSP, cwgsp); s.rw(MODGSP, modgsp); s.rw(GSNOFF, gsnoff)
        s.rw(RFCFG, gain); s.rw(RXTHRESH, rxth); s.rm(STATUS2, 0x08, RESET)
...
def sweep(name, ch):
    ...
    for cw in ('both', 'cw1', 'cw2'):
        for gsnon in (0xF0, 0xC8, 0x88, 0x48):       # overall N-driver drive (lower = weaker field)
            for cwgsp in (0x3F, 0x20, 0x10, 0x08):   # P-driver CW conductance
                for modgsp in (0x20, 0x10, 0x08):    # modulation depth
                    gsnoff = 0x00
                    uids = {}
                    for _ in range(3):
                        fm.reset(); fm.coil(upper); fm.init(gsnon, cwgsp, modgsp, gsnoff)
                    ...
                    if len(stable) == 1:
                        line = "REAL %s cw=%s gsnon=0x%02X cwgsp=0x%02X modgsp=0x%02X -> UID [%s] x%d/3" % (
                            name, cw, gsnon, cwgsp, modgsp, stable[0][0], stable[0][1])
